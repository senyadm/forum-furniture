...
export default function Navbar() {
  ...
  // Определяем активный пункт только по текущему роуту
  useEffect(() => {
    if (pathname === "/") {
      setActiveKey("home");
    } else if (pathname.startsWith("/mebel")) {
      setActiveKey("mebel");
    } else if (pathname.startsWith("/remont")) {
      setActiveKey("remont");
  ...
  // скрытие/появление при скролле
  useEffect(() => {
    const handleScroll = () => {
      const current = window.scrollY;

      if (current > lastScroll && current > 70) {
        setHidden(true); // вниз — спрятать капсулу
      } else {
        setHidden(false); // вверх — показать
      }
